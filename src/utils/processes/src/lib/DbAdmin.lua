-- Based on the original code from (with some improvements):
-- https://github.com/twilson63/aos-packages/blob/main/packages/db-admin/src/DbAdmin.lua
...
function dbAdmin:execMulti(sql)
    local result = self.db:exec(sql)
    if result ~= 0 then
        error("Multiple statement execution failed: " .. self.db:errmsg())
    end
    return true
end
